...
if page == "Dashboard":
    ...
    if not vuln_df.empty and not advisory_df.empty:
        # Map advisory info to vulnerabilities
        ...
        # Add empty columns if no advisory data
        vuln_df['advisory_title'] = ''
        vuln_df['cwe'] = ''
        vuln_df['infra_sector'] = ''

    # Merge vulnerabilities with assets
    if not vuln_df.empty and not assets_df.empty:
    ...
    def risk_score(row):
        ...
        factor = crit_map.get(str(row['criticality']), 1)
        return row['cvss_score'] * factor if pd.notna(row['cvss_score']) else 0
    ...
    with col4:
        critical_count = len(assets_df[assets_df['criticality'] == 'Critical']) if not assets_df.empty else 0
        st.metric("Critical Assets", critical_count)
    with col5:
        risk_total = calculate_risk_score(assets_df, vuln_df)
        st.metric("Total Risk Score", f"{risk_total:.0f}")

    # Filters
    st.markdown("### Filters")
    filter_col1, filter_col2 = st.columns(2)
    with filter_col1:
        criticality_options = assets_df['criticality'].unique() if not assets_df.empty else []
        criticality_filter = st.multiselect("Asset Criticality", options=criticality_options, default=criticality_options)
    with filter_col2:
        zone_options = assets_df['network_zone'].unique() if not assets_df.empty else []
        zone_filter = st.multiselect("Network Zone", options=zone_options, default=zone_options)

    # Additional filters
    if not merged_df.empty:
        asset_type_options = merged_df['asset_type'].dropna().unique()
        exploit_options = merged_df['exploitability'].dropna().unique()
        patch_options = merged_df['patch_availability'].dropna().unique()
        cwe_options = merged_df['cwe'].dropna().unique()
        sector_options = merged_df['infra_sector'].dropna().unique()
    else:
        asset_type_options = exploit_options = patch_options = cwe_options = sector_options = []

    with st.expander("Additional Filters"):
        col_a, col_b = st.columns(2)
        with col_a:
            selected_asset_types = st.multiselect("Asset Type", options=asset_type_options, default=asset_type_options)
            selected_exploit = st.multiselect("Exploitability", options=exploit_options, default=exploit_options)
            selected_cwes = st.multiselect("CWE", options=cwe_options, default=cwe_options)
        with col_b:
            selected_patch = st.multiselect("Patch Availability", options=patch_options, default=patch_options)
            selected_sectors = st.multiselect("Infrastructure Sector", options=sector_options, default=sector_options)
            cvss_range = st.slider("CVSS Score Range", 0.0, 10.0, (0.0, 10.0))
            risk_range = st.slider("Risk Score Range", 0.0, 40.0, (0.0, 40.0))
    ...
    if not filtered_df.empty:
        ...
        if zone_filter:
            filtered_df = filtered_df[filtered_df['network_zone'].isin(zone_filter)]
        if selected_asset_types:
            filtered_df = filtered_df[filtered_df['asset_type'].isin(selected_asset_types)]
        if selected_exploit:
            filtered_df = filtered_df[filtered_df['exploitability'].isin(selected_exploit)]
        if selected_patch:
            filtered_df = filtered_df[filtered_df['patch_availability'].isin(selected_patch)]
        if selected_cwes:
            filtered_df = filtered_df[filtered_df['cwe'].isin(selected_cwes)]
        if selected_sectors:
            filtered_df = filtered_df[filtered_df['infra_sector'].isin(selected_sectors)]
        filtered_df = filtered_df[filtered_df['cvss_score'].between(cvss_range[0], cvss_range[1])]
        filtered_df = filtered_df[filtered_df['risk_score'].between(risk_range[0], risk_range[1])]

    if filtered_df.empty:
        st.warning("No data matches the selected filters. Please adjust filters or add data.")
    else:
        ...
        with tab_overview:
            # Row 1: CVSS distribution and severity pie
            ...
            # Row 2: Risk score distribution and criticality bar
            col1, col2 = st.columns(2)
            with col1:
            ...
            if not zone_risk.empty:
                fig_zone = px.bar(zone_risk, x='network_zone', y='risk_score', title="Average Risk Score by Network Zone",
        ...
        with tab_asset_risk:
            ...
            st.markdown("Explore vulnerabilities per asset. Expand any asset to see details.")
            ...
            for asset_id, group in asset_groups:
                ...
                with st.expander(f"🏭 Asset ID: {asset_id} | Type: {asset_type} | Criticality: {criticality} | Avg Risk: {avg_risk:.2f}"):
                    ...
                    # Table of vulnerabilities for this asset
        ...
        with tab_vuln_insights:
            ...
            fig_cvss_risk = px.scatter(filtered_df, x='cvss_score', y='risk_score',
                                       color='severity', size='criticality_factor' if 'criticality_factor' in filtered_df.columns else 'risk_score',
                                       hover_data=['cve_id', 'asset_id'],
            ...
            # Vulnerability timeline (if created_at present)
            if 'created_at_x' in filtered_df.columns:
                st.subheader("Vulnerability Timeline")
                filtered_df['date'] = pd.to_datetime(filtered_df['created_at_x']).dt.date
    ...
    if advisory_file is not None:
        try:
            ...
            if missing:
                ...
                # Optional columns: 'ics-cert_advisory_title', 'cwe_number', 'critical_infrastructure_sector'
...
# Import Data
    ...
    st.markdown("Use the standard templates below to import assets, vulnerabilities, and advisory data.")
    ...
    st.markdown("---")
    st.subheader("Upload Files to Import")
